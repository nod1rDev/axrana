...
export default function MenuBatalyon10({ data }: { data: any }) {
  ...
  return (
    <>
      <h1 className="text-[28px] mt-10 font-bold mx-auto text-center mb-4">
        {latinToCyrillic("Yuriqnoma tanlang tanlang")}
      </h1>
      ...
      <div className="w-[80%] mb-10 mx-auto flex flex-col gap-4">
        {itemData.map((item: any, index: number) => (
          ...
          >
            <h1 className="text-[20px] font-bold text-center">{item.title}</h1>
          </button>
        ))}
      </div>
    </>
